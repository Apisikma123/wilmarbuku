...
<div class="px-6 md:px-12 xl:px-24 max-w-[1200px] mx-auto py-12">
    <!-- Back Button -->
    <div class="mb-6">
        <a href="javascript:void(0)" onclick="if(document.referrer) { window.history.back(); } else { window.location.href='/cart'; }" class="inline-flex items-center text-sm font-medium text-on-surface-variant hover:text-primary transition-colors">
        ...
        <div class="w-full lg:w-[60%] flex flex-col gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-outline-variant/30 p-6 md:p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <!-- Internal -->
                    <label class="relative flex flex-col border border-outline-variant/50 rounded-xl p-4 cursor-pointer transition-colors peer-checked:border-[#004225] peer-checked:border-2" id="label-internal">
                    ...
                    <!-- Eksternal -->
                    <label class="relative flex flex-col border border-outline-variant/50 rounded-xl p-4 cursor-pointer transition-colors peer-checked:border-[#004225] peer-checked:border-2" id="label-eksternal">
                ...
                <!-- Form Fields -->
                <div class="space-y-5">
                    <div id="identitas-kampus-container" class="{{ Auth::user()->role == 'user_internal' ? '' : 'hidden' }}">
                        <label class="block text-sm font-medium text-on-surface mb-1">Nomor Induk Mahasiswa / NIDN <span class="text-red-500">*</span></label>
                        <input type="text" name="identitas_kampus" value="{{ Auth::user()->identitas_kampus }}" class="w-full px-4 py-3 bg-[#f8fafc] border border-outline-variant/30 rounded-lg focus:border-[#004225] focus:ring-1 focus:ring-[#004225] outline-none transition-colors text-sm" placeholder="Masukkan NIM/NIDN Anda">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" name="nama_lengkap" value="{{ Auth::user()->nama_lengkap }}" class="w-full px-4 py-3 bg-[#f8fafc] border border-outline-variant/30 rounded-lg focus:border-[#004225] focus:ring-1 focus:ring-[#004225] outline-none transition-colors text-sm" placeholder="Nama sesuai identitas" required>
                            @error('nama_lengkap') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-on-surface mb-1">Email <span class="text-red-500">*</span></label>
                            <input type="email" name="email" value="{{ Auth::user()->email }}" class="w-full px-4 py-3 bg-[#f8fafc] border border-outline-variant/30 rounded-lg outline-none transition-colors text-sm text-gray-500 cursor-not-allowed" placeholder="alamat@email.com" readonly>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="flex flex-col sm:flex-row gap-6 mb-6">
                    ...
                    <div class="flex flex-col justify-center">
                        ...
                        <div class="flex gap-2 mb-2">
                            <span class="bg-[#f1f5f9] text-[#475569] text-[11px] font-bold px-3 py-1.5 rounded-md">{{ $item['kategori'] }}</span>
                        </div>
                        <p class="text-sm font-bold text-primary">{{ $item['qty'] }} Buku x Rp {{ number_format($item['harga_estimasi'], 0, ',', '.') }}</p>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-outline-variant/30 overflow-hidden sticky top-24">
                ...
                <div class="p-6">
                    ...
                    <div class="flex justify-between items-center border-t border-outline-variant/30 pt-6 mb-8">
                        ...
                        <span class="font-bold text-2xl text-[#004225]">Rp {{ number_format($total, 0, ',', '.') }}</span>
                    </div>

                    <button type="submit" class="w-full bg-[#004225] text-white font-bold text-sm px-6 py-4 rounded-xl hover:bg-primary transition-colors shadow-md flex justify-center items-center gap-2 mb-4">
