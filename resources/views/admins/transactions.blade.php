...
@section('content')
    <div class="max-w-7xl mx-auto space-y-6 p-4 md:p-0">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-slate-500 text-sm mt-1">Konfirmasi pembayaran, update status pengiriman, dan kirim
                    pemberitahuan ke donatur.</p>
            </div>
            <button onclick="document.getElementById('modal-metode-pembayaran').classList.remove('hidden')"
                class="bg-[#003215] hover:bg-[#004b23] text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-sm flex items-center justify-center gap-2 transition-colors">
        ...
        @if(session('success'))
            <div
                ...
                <button onclick="this.parentElement.remove()" class="text-emerald-500 hover:text-emerald-700"><i data-lucide="x"
        ...
        @if(session('error'))
            <div
                class="bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-xl flex items-center justify-between shadow-sm">
                <div class="flex items-center gap-3">
                    <i data-lucide="alert-circle" class="w-5 h-5 text-red-600"></i>
                    <span class="text-sm font-bold">{{ session('error') }}</span>
                </div>
                <button onclick="this.parentElement.remove()" class="text-red-500 hover:text-red-700"><i data-lucide="x"
                        class="w-4 h-4"></i></button>
            </div>
        @endif
        ...
        <div
            class="bg-gradient-to-br from-green-900 via-green-800 to-green-950 rounded-2xl p-8 lg:p-10 shadow-xl text-white flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden border border-green-800">
            <div
                class="absolute right-0 top-0 w-80 h-80 bg-green-400 opacity-10 rounded-full blur-3xl -translate-y-1/3 translate-x-1/4 pointer-events-none">
            </div>
            <div
                class="absolute left-0 bottom-0 w-64 h-64 bg-green-500 opacity-10 rounded-full blur-3xl translate-y-1/3 -translate-x-1/4 pointer-events-none">
            </div>

            <div
                class="relative z-10 flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-6 w-full">
                <div
                    class="w-20 h-20 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20 shadow-inner shrink-0">
                    <i data-lucide="wallet" class="w-10 h-10 text-green-100"></i>
                </div>
                <div>
                    <h3 class="text-green-200 font-bold uppercase tracking-widest text-xs mb-1.5">Total Donasi Terkumpul
                    </h3>
                    <div class="flex flex-col sm:flex-row sm:items-end gap-3 sm:gap-4 mt-2">
                        <span
                            class="text-4xl md:text-5xl lg:text-6xl font-black tracking-tight text-white drop-shadow-sm">Rp
                            {{ number_format($totalDonations, 0, ',', '.') }}</span>
                        <span
                            class="text-sm font-bold text-green-100 mb-2 inline-flex items-center gap-1.5 bg-green-950/40 border border-green-700/50 px-2.5 py-1 rounded-lg backdrop-blur-sm self-center sm:self-auto">
                            <i data-lucide="trending-up" class="w-4 h-4 text-green-400"></i> Realtime Status
                        </span>
                    </div>
                    <p class="text-green-100/70 text-sm mt-3 max-w-md">Total nominal donasi dari transaksi terverifikasi
                        dalam sistem.</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-6">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                @forelse($metodes as $metode)
                    <div
                        class="border border-slate-200 rounded-xl p-4 flex items-center justify-between group hover:border-green-600/50 transition-colors">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-12 h-12 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center font-bold text-sm uppercase">
                        ...
                        <form action="{{ route('admin.metode.destroy', $metode->id) }}" method="POST"
                            ...
                            @method('DELETE')
                            <button type="button"
                                class="w-8 h-8 rounded-full text-slate-400 hover:text-red-500 hover:bg-red-50 flex items-center justify-center transition-colors btn-delete-metode">
                ...
                @empty
                    <div
                        class="col-span-full py-6 text-center text-slate-400 text-sm italic border border-dashed border-slate-200 rounded-xl">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            <a href="{{ route('admin.transactions', array_merge(request()->except('page'), ['status' => request('status') == 'Unpaid' ? null : 'Unpaid'])) }}"
                ...
                <p class="text-[10px] font-bold text-slate-500 mb-1 leading-tight uppercase tracking-wider">Belum Dibayar
                </p>
                <h3 class="text-xl font-bold text-slate-900">{{ number_format($pendingPayments) }}</h3>
            </a>

            <a href="{{ route('admin.transactions', array_merge(request()->except('page'), ['status' => request('status') == 'DanaDiterima' ? null : 'DanaDiterima'])) }}"
                class="bg-white rounded-xl border {{ request('status') == 'DanaDiterima' ? 'border-indigo-500 ring-2 ring-indigo-100' : 'border-slate-200 hover:border-indigo-300' }} p-4 shadow-sm flex flex-col justify-center transition-all cursor-pointer">
                <div class="flex items-center gap-2 mb-2">
                    <div class="w-7 h-7 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600">
                        <i data-lucide="wallet" class="w-3.5 h-3.5"></i>
                    </div>
                </div>
                <p class="text-[10px] font-bold text-slate-500 mb-1 leading-tight uppercase tracking-wider">Dana Diterima
                </p>
                <h3 class="text-xl font-bold text-slate-900">{{ number_format($danaDiterima) }}</h3>
            </a>

            <a href="{{ route('admin.transactions', array_merge(request()->except('page'), ['status' => request('status') == 'InProcess' ? null : 'InProcess'])) }}"
                class="bg-white rounded-xl border {{ request('status') == 'InProcess' ? 'border-blue-500 ring-2 ring-blue-100' : 'border-slate-200 hover:border-blue-300' }} p-4 shadow-sm flex flex-col justify-center transition-all cursor-pointer">
                <div class="flex items-center gap-2 mb-2">
            ...
            <a href="{{ route('admin.transactions', array_merge(request()->except('page'), ['status' => request('status') == 'Completed' ? null : 'Completed'])) }}"
                class="bg-white rounded-xl border {{ request('status') == 'Completed' ? 'border-emerald-500 ring-2 ring-emerald-100' : 'border-slate-200 hover:border-emerald-300' }} p-4 shadow-sm flex flex-col justify-center transition-all cursor-pointer">
                <div class="flex items-center gap-2 mb-2">
            ...
            <a href="{{ route('admin.transactions', array_merge(request()->except('page'), ['status' => request('status') == 'Cancelled' ? null : 'Cancelled'])) }}"
                class="bg-white rounded-xl border {{ request('status') == 'Cancelled' ? 'border-red-500 ring-2 ring-red-100' : 'border-slate-200 hover:border-red-300' }} p-4 shadow-sm flex flex-col justify-center transition-all cursor-pointer">
                <div class="flex items-center gap-2 mb-2">
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden mb-6">
            <form method="GET" action="{{ route('admin.transactions') }}"
                class="px-6 py-5 border-b border-slate-200 flex flex-col sm:flex-row sm:justify-between sm:items-center bg-slate-50/50 flex-wrap gap-4">
                <h3 class="text-lg font-bold text-slate-900 shrink-0">Daftar Transaksi</h3>
                <div class="relative w-full sm:w-auto shrink-0 flex items-center gap-2">
                    ...
                    <button type="submit"
                        class="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-green-700 transition-colors shrink-0">Cari</button>
                </div>
            </form>
            ...
            <div id="transactions-table-container" class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead
                        class="text-[10px] text-slate-500 font-black uppercase tracking-widest bg-slate-50/50 border-b border-slate-200">
                        <tr>
                                            ...
                                            @foreach($trx->details as $detail)
                                                <li class="border-b border-slate-100 pb-2 last:border-0 last:pb-0">
                                                    ...
                                                    @if($detail->pesan_dukungan && !in_array($trx->status_tracking, ['Menunggu Pembayaran']))
                                                        <div
                                                            class="mt-1.5 ml-2 p-2 bg-blue-50 border-l-2 border-blue-400 rounded-r text-[10px] text-slate-600 italic">
                                                            <i data-lucide="message-square"
                                                                class="w-3 h-3 inline mr-1 text-blue-500"></i>
                                                            "{{ $detail->pesan_dukungan }}"
                                                        </div>
                                                    @endif
                                                </li>
                                            @endforeach
                                ...
                                <td class="px-6 py-5">
                                    <div class="flex flex-col gap-1">
                                        <div class="flex items-center gap-2">
                                            <div
                                                class="w-2.5 h-2.5 rounded-full {{ $trx->status_tracking == 'Selesai' ? 'bg-emerald-500' : ($trx->status_tracking == 'Dibatalkan' ? 'bg-red-500' : ($trx->status_tracking == 'Dalam Pengiriman' ? 'bg-blue-500' : 'bg-amber-500')) }}">
                                            </div>
                                        ...
                                        @if($trx->status_tracking == 'Dibatalkan' && $trx->alasan_pembatalan)
                                            <p
                                                class="text-[11px] text-red-600 bg-red-50 p-1.5 rounded border border-red-100 mt-1 max-w-xs">
                                    ...
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        @if(in_array($trx->status_tracking, ['Selesai', 'Dibatalkan']) || in_array($trx->status_pembayaran, ['Failed', 'Expired']))
                                            @if($trx->status_tracking == 'Selesai')
                                                <span
                                                    class="px-3 py-1.5 bg-slate-100 text-slate-400 rounded-lg text-xs font-bold flex items-center gap-1">
                                                    <i data-lucide="check-circle-2" class="w-3.5 h-3.5"></i> Telah Selesai
                                                </span>
                                            @else
                                                <span
                                                    class="px-3 py-1.5 bg-red-50 text-red-400 rounded-lg text-xs font-bold flex items-center gap-1 border border-red-100">
    ...
    <div id="statusModal"
        class="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm hidden flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-200">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
                ...
                <button onclick="closeStatusModal()" class="text-slate-400 hover:text-slate-600"><i data-lucide="x"
            ...
            <form id="statusForm" method="POST" class="space-y-4">
                ...
                <div id="buktiPembayaranContainerStatus" class="hidden mb-4">
                    ...
                    <a id="buktiPembayaranLinkStatus" href="#" target="_blank"
                        class="block group relative rounded-lg border border-slate-200 overflow-hidden bg-slate-50">
                        <img id="buktiPembayaranImgStatus" src="" alt="Bukti Pembayaran"
                            class="w-full h-auto max-h-60 object-contain group-hover:opacity-80 transition-opacity">
                        <div
                            class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <span
                                class="bg-black/70 text-white px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5"><i
                                    data-lucide="external-link" class="w-3.5 h-3.5"></i> Buka Penuh</span>
                        </div>
                    </a>
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase text-slate-600 mb-1">Pilih Status Baru *</label>
                    <select id="modal_status_tracking" name="status_tracking" required
                        class="w-full border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm outline-none focus:border-green-600 bg-white">
                ...
                <div>
                    <label class="block text-xs font-bold uppercase text-slate-600 mb-1">Pesan Khusus ke Inbox Donatur
                        (Opsional)</label>
                    <textarea name="pesan_admin" rows="3"
                        class="w-full border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm outline-none focus:border-green-600 focus:ring-1 focus:ring-green-600"
    ...
    <div id="modal-metode-pembayaran"
        ...
        <div class="bg-white rounded-2xl max-w-md w-full overflow-hidden shadow-2xl relative">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                ...
                <button onclick="document.getElementById('modal-metode-pembayaran').classList.add('hidden')"
                    class="w-8 h-8 flex items-center justify-center rounded-full text-slate-400 hover:text-slate-600 hover:bg-slate-200/50 transition-colors">
            ...
            <form action="{{ route('admin.metode.store') }}" method="POST" class="p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1.5">Metode Apa yang Mau
                            Ditambah?</label>
                        <select name="tipe"
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_bank" placeholder="Contoh: BCA, Mandiri, BNI" required
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:ring-green-600 focus:border-green-600">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="atas_nama" placeholder="Contoh: Admin WilmarBOOKS" required
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:ring-green-600 focus:border-green-600">
                    </div>
                </div>
                ...
                <div class="mt-8 flex gap-3">
                    <button type="button"
                        onclick="document.getElementById('modal-metode-pembayaran').classList.add('hidden')"
                        class="flex-1 bg-white border border-slate-200 text-slate-600 font-bold py-2.5 rounded-xl hover:bg-slate-50 transition-colors">Batal</button>
                    <button type="submit"
                        class="flex-1 bg-[#003215] text-white font-bold py-2.5 rounded-xl hover:bg-[#004b23] transition-colors shadow-sm">Simpan</button>
                </div>
            </form>
        </div>
    </div>
