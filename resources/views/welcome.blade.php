...
@section('content')
    <!-- Hero Section -->
    <section
        class="bg-primary relative px-6 md:px-12 py-12 md:py-24 overflow-hidden flex items-center min-h-[500px] md:min-h-[700px]">
        <!-- Background Image with Overlay -->
        <div class="absolute inset-0 z-0">
            <img alt="Wilmar Building" class="w-full h-full object-cover object-center"
        ...
        <div class="relative z-10 max-w-7xl mx-auto w-full grid md:grid-cols-2 gap-12 items-center">
            <div class="space-y-8">
                <h1 class="text-5xl md:text-6xl lg:text-7xl font-bold text-white leading-tight tracking-tight">Donasikan
                    Buku,<br />Kembangkan Literasi Di Wilmar</h1>
                <p class="text-lg md:text-xl text-white/90 max-w-lg leading-relaxed font-light">
                    Pilih buku dari katalog, bayar online, dan buku langsung dikirim ke perpustakaan kampus WBI. Donasi Anda
                    jadi referensi belajar mahasiswa — bukan cuma angka di laporan.
                </p>
                <div class="flex flex-wrap gap-4 pt-4">
                    @if(!auth()->check() || auth()->user()->role !== 'admin')
                        <a href="{{ Auth::check() ? route('dashboard') : route('login') }}"
                            class="bg-secondary text-on-secondary font-semibold px-8 py-4 rounded-md hover:bg-secondary-fixed transition-colors shadow-lg inline-flex items-center gap-2 justify-center">
    ...
    <section class="bg-surface py-10 md:py-16 border-b border-outline-variant/30">
        <div class="px-6 md:px-12 max-w-7xl mx-auto">
            <div class="grid grid-cols-3 gap-2 md:gap-8 divide-x divide-outline-variant/30">
                <div
                    class="py-4 md:py-0 px-2 md:px-6 flex flex-col md:flex-row items-center justify-center md:justify-start text-center md:text-left gap-2 md:gap-6">
                    <div
                        class="w-10 h-10 md:w-16 md:h-16 bg-surface-container-low rounded-full flex items-center justify-center flex-shrink-0">
                        <span class="material-symbols-outlined text-xl md:text-3xl text-primary">menu_book</span>
                    </div>
                    <div>
                        <p class="text-xl sm:text-2xl md:text-4xl font-bold text-primary mb-0 md:mb-1 tracking-tight">
                            {{ number_format($global_total_buku, 0, ',', '.') }}+</p>
                        <p
                            class="text-[9px] sm:text-[10px] md:text-sm font-medium text-on-surface-variant uppercase tracking-wider leading-tight">
                            Buku Terkumpul</p>
                    </div>
                </div>
                <div
                    ...
                    <div>
                        <p class="text-xl sm:text-2xl md:text-4xl font-bold text-primary mb-0 md:mb-1 tracking-tight">
                            {{ number_format($global_donatur_aktif, 0, ',', '.') }}+</p>
                        <p
                            class="text-[9px] sm:text-[10px] md:text-sm font-medium text-on-surface-variant uppercase tracking-wider leading-tight">
                            Donatur Aktif</p>
                    </div>
                ...
                <div
                    ...
                    <div>
                        <p class="text-xl sm:text-2xl md:text-4xl font-bold text-primary mb-0 md:mb-1 tracking-tight">100%
                        </p>
                        <p
                            class="text-[9px] sm:text-[10px] md:text-sm font-medium text-on-surface-variant uppercase tracking-wider leading-tight">
                            Transparansi Laporan</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="px-6 md:px-12 max-w-7xl mx-auto py-12 md:py-24">
        <div class="grid md:grid-cols-2 gap-10 md:gap-20 items-center">
            <div
                class="order-2 md:order-1 relative rounded-xl overflow-hidden aspect-[4/3] max-w-xl mx-auto w-full shadow-2xl shadow-primary/10">
                <div class="bg-cover bg-center w-full h-full"
                    data-alt="A close-up shot of hands exchanging a stack of high-quality academic and business books. The lighting is warm and inviting, highlighting the crisp pages and professional covers. The setting feels like a modern academic institution with subtle forest green branding in the background. The mood is collaborative and empowering."
                    style="background-image: url('{{ asset('images/perpus.png') }}')"></div>
            </div>
            <div class="order-1 md:order-2 space-y-8">
                <h2 class="text-4xl md:text-5xl font-bold text-primary leading-tight tracking-tight">Nurturing Entrepreneurs
                    through Literacy</h2>
                <div class="space-y-6 text-lg text-on-surface-variant leading-relaxed">
                    <p class="">
                        Perpustakaan WBI melayani 300+ mahasiswa aktif, tapi koleksi bukunya belum memadai. Donasi Anda
                        langsung mengisi rak yang kosong — buku bisnis, teknologi, sampai sastra.
                    </p>
                    <p class="">
...
<div class="absolute -right-20 -top-20 w-64 h-64 bg-primary-container/30 blur-[80px] rounded-full mix-blend-screen hidden md:block"></div>
            ...
            <div class="relative z-10 max-w-2xl mx-auto flex flex-col items-center">
                ...
                <p class="text-sm md:text-lg text-white/90 mb-8 md:mb-10 font-light leading-relaxed">
                    Perpustakaan kampus butuh buku baru tiap semester. Satu donasi Anda bisa dibaca puluhan mahasiswa selama
                    bertahun-tahun.
                </p>
                @if(!auth()->check() || auth()->user()->role !== 'admin')
                    <a href="{{ Auth::check() ? route('dashboard') : route('login') }}"
                        class="bg-secondary text-on-secondary font-bold px-8 py-3 md:px-10 md:py-4 rounded-md hover:bg-secondary-fixed transition-colors shadow-lg text-sm md:text-lg inline-block text-center">
                        Donasi Sekarang
                    </a>
                @endif
            </div>
