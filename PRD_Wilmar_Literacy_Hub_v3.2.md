...
| `midtrans_id` | Varchar | Nullable, ID Transaksi dari Midtrans |
| `status_pembayaran`| Enum | 'Unpaid', 'Paid', 'Expired', 'Failed' |
| `status_tracking`| Enum | 'Menunggu Pembayaran', 'Dana Diterima', 'Dipesan Admin', 'Dikirim ke Perpus', 'Masuk Katalog' |
| `validasi_lulus` | Boolean | `true` jika status_tracking mencapai 'Masuk Katalog' (Khusus mempermudah filter mahasiswa lulus) |
...
| `harga_estimasi`| Decimal/Int| Harga buku untuk acuan checkout via Midtrans |
| `status_buku` | Enum | 'Dibutuhkan' (Bisa di-checkout), 'Tersedia' (Sudah di rak) |
| `donatur_id` | FK (Varchar) | Relasi ke `kode_tracking` untuk menampilkan nama penyumbang di detail katalog |
